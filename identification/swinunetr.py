...
class SwinUNETR(nn.Module):
    ...
    def __init__(
            ...
            num_heads: Tuple[int, int, int, int] = [3, 6, 6, 6],
        ...
        # self.out = UnetOutBlock(spatial_dims=3, in_channels=feature_size, out_channels=classes)  # type: ignore
        self.out = nn.Linear(feature_size*16, classes)  # type: ignore
        self.conv1 = nn.Conv3d(3, 3, kernel_size=(7, 7, 7), stride=(2, 2, 2), padding=(3, 3, 3), bias=False)
    ...
    def forward(self, x_in):
        ...
        # logits = self.out(out)
        return self.out(self.avg_pool(hidden_states_out[-1]).view(hidden_states_out[-1].size(0), -1))



if __name__ == "__main__":
    net_3d = SwinUNETR(img_size=(64, 64, 64), in_channels=3, classes=5).cuda()
    t1 = torch.rand(1, 3, 64, 64, 64).cuda()
    out = net_3d(t1)
    print(out.shape)
